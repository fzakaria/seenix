...
// Tests the way back: encoding the decoded bytes must reproduce the
// string nix wrote, and every spelling of a NAR hash that nix path-info
// has used must normalise to the narinfo form a signature covers.
import { encodeNixBase32, normalizeHash } from "../../site/js/hash.js";

test("nix base32 encodes back to the string nix wrote", () => {
  assert.equal(encodeNixBase32(decodeNixBase32(HELLO_NIX32, 32)), HELLO_NIX32);
});

test("SRI, hex and nix32 hashes normalise to the narinfo form", () => {
  const want = `sha256:${HELLO_NIX32}`;
  assert.equal(normalizeHash(`sha256-${HELLO_BASE64}`), want);
  assert.equal(normalizeHash(`sha256:${HELLO_HEX}`), want);
  assert.equal(normalizeHash(want), want);
});
