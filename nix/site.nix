...
pkgs.runCommand "seenix-site" { } ''
  mkdir -p $out
  # The site directory alone, so a change to a test or a doc does not
  # rebuild the site.
  cp -r ${../site}/. $out/
  chmod -R u+w $out
  rm -rf $out/vendor
  ...
  # The footer names the store path serving the page.
  substituteInPlace $out/js/app.js --replace-fail "__STORE_PATH__" "$out"

  # Hash the module tree and rename it js.<hash>, so the served HTML and
  # every module and worker it pulls in can never be a mismatched pair
  # across deploys. Modules and workers load each other by relative URL,
  # and vendor/ is reached as ../vendor from inside the tree.
